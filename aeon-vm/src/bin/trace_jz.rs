fn main() {
    ...
    use aeon_vm::vm::{VMState, StepResult};
    ...
    for i in 0..10 {
        if s.pc >= p.instructions.len() { println!("pc OOB {}", s.pc); break; }
        println!("step {} pc={} inst={}", i, s.pc, p.instructions[s.pc].disassemble());
        let res = s.step(&p);
        println!(" regs[0..3]={:?} result={:?}", &s.regs[0..3], res);
        if let StepResult::Halted = res { println!("halted"); break; }
    }
}
