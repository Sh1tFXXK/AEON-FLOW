...
impl CollabDoc {
    ...
    /// Apply a serialized document snapshot (or merged state) received from another device.
    ...
    /// Current text content in the collaborative document.
    ...
    /// Insert text locally at a UTF-8 byte position.
    pub fn insert(&mut self, pos: usize, text: &str) {
        ...
        if let Some((_, obj)) = self.doc.get(automerge::ROOT, "content").ok().flatten() {
            let safe_pos = pos.min(self.content().len());
            self.doc
                .splice_text(&obj, safe_pos, 0, text)
                .expect("local insert should be valid");
            self.change_count += 1;
            self.refresh_cid();
        }
    }
