...
async fn main() {
    ...
    let identity =
        Identity::load_or_create(&home.join(".aeon").join("identity")).expect("identity");

    let discovery = discovery::Discovery {
        identity_id: identity.id,
        port: 8787,
    };
    let mdns = discovery.try_announce();
    ...
    let device_name =
        std::env::var("AEON_DEVICE_NAME").unwrap_or_else(|_| "local-device".to_string());
    ...
    let listen_addr =
        std::env::var("AEON_AGENT_LISTEN").unwrap_or_else(|_| "0.0.0.0:8787".to_string());
    let accept_engine = engine.clone();
    tokio::spawn(async move {
        let _ = accept_engine.listen(&listen_addr).await;
    });
        ...
        tokio::spawn(async move {
            loop {
                let healthy = tunnel::health_check(&tunnel_cfg.endpoint).await;
                {
                    ...
                    st.set_tunnel_status(
                        tunnel_cfg.provider.clone(),
                        tunnel_cfg.endpoint.clone(),
                        healthy,
                        if healthy {
                            "healthy".to_string()
                        } else {
                ...
                tokio::time::sleep(std::time::Duration::from_secs(
                    tunnel_cfg.health_interval_secs,
                ))
                .await;
            }
        });
    ...
    let file_index: Arc<Mutex<HashMap<String, [u8; 32]>>> =
        Arc::new(Mutex::new(engine.state.lock().unwrap().tombstone_map()));
                        ...
                        if let Ok(signed) = signed {
                            ...
                            if blob.mime.starts_with("text/") || blob.mime == "application/json" {
                                ...
                                let (doc_id, changes) = {
                                    ...
                                    st.save(&engine.state_path);
                                    let mut doc = collab::CollabDoc::new(&String::from_utf8_lossy(
                                        &blob.data,
                                    ));
                                    (doc_id, doc.save())
                                };
